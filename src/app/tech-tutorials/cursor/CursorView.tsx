...
const CATEGORIES = [
  ...
  "Composer, Chat & Prompting",
  "Rules, Memory & Agent Workflows",
  "Codebase, Refactor & Debugging",
  "MCP, Extensions & Integrations",
  "Frameworks & Real Projects",
...
export default function CursorView({ entries }: Props) {
      ...
      <div className="flex-1 min-w-0 px-5 pr-12 py-8 md:px-8 md:pr-14">
        ...
        <div className="mb-6 space-y-1.5">
          <div className="font-mono text-[9px] tracking-[0.25em] text-gold/70 uppercase">
            [ TECH TUTORIALS // CURSOR ]
          </div>
          <p className="font-sans text-[12px] text-charcoal/50 max-w-2xl leading-relaxed">
            106 community tutorials covering Cursor setup, Composer, AI agent workflows, codebase navigation, MCP integrations, real projects, and productivity tips.
          </p>
        </div>
              ...
              <section className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {startHereVideos.map((entry, idx) => (
                    <VideoCard key={`start-${entry.link}-${idx}`} entry={entry} source="Cursor" />
                  ))}
                </div>
              </section>
            )}
                ...
                <section key={category} className="space-y-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {catEntries.map((entry, idx) => (
                      <VideoCard key={`${entry.link}-${idx}`} entry={entry} source="Cursor" />
                    ))}
                  </div>
                </section>
              );
