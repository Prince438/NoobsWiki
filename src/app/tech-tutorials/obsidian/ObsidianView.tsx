...
const SKIP_TAG = "obsidian";

const CATEGORIES = [
  ALL,
  "Beginner / Start Here",
  "Core Setup, Markdown & Notes",
  "Plugins, Properties & Vault Organization",
  "Links, Backlinks, Search & Graph",
  "Daily Notes, Templates & Tasks",
  "Dataview, Properties & Bases",
  "Canvas, Zettelkasten, Second Brain & AI",
  "Mobile, Sync & Publish",
];
...
export default function ObsidianView({ entries }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (levelFilter !== "All") {
      result = result.filter((e) => e.beginnerFriendly === levelFilter);
    }
    if (formatFilter !== "All") {
  ...
  // "Start Here" block — shown at top when All + no filters active
      ...
      <div className="flex-1 min-w-0 px-5 pr-12 py-8 md:px-8 md:pr-14">
        ...
        <div className="mb-6 space-y-1.5">
          <div className="font-mono text-[9px] tracking-[0.25em] text-gold/70 uppercase">
            [ TECH TUTORIALS // OBSIDIAN ]
          </div>
          <p className="font-sans text-[12px] text-charcoal/50 max-w-2xl leading-relaxed">
            76 community tutorials covering setup, plugins, vault organisation, daily notes, Dataview, Canvas, and more — for every level of Obsidian user.
          </p>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-3 rounded border border-panel-border bg-panel p-3.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Format filter — only shown if Shorts exist in the data */}
            {hasFormats && (
              <div className="flex items-center gap-1.5">
