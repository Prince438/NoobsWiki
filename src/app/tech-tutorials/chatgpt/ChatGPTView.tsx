...
const CATEGORIES = [
  ALL,
  "Beginner / Start Here",
  "Prompting & Everyday Use",
  "Images, Voice & Multimodal",
  "Projects, Memory & Custom GPTs",
  "Coding, API & Automation",
  "Workflow, Study & Business",
  "Reviews, Comparisons & Updates",
];

type LevelFilter = "All" | "Yes" | "Somewhat" | "No";
type FormatFilter = "All" | "Full video" | "Short";

export default function ChatGPTView({ entries }: Props) {
      ...
      <div className="flex-1 min-w-0 px-5 pr-12 py-8 md:px-8 md:pr-14">
        ...
        <div className="mb-6 space-y-1.5">
          <div className="font-mono text-[9px] tracking-[0.25em] text-gold/70 uppercase">
            [ TECH TUTORIALS // CHATGPT ]
          </div>
          <p className="font-sans text-[12px] text-charcoal/50 max-w-2xl leading-relaxed">
            67 community tutorials covering ChatGPT basics, prompting, images and voice, custom GPTs, coding, automation, and productivity workflows.
          </p>
        </div>
              ...
              <section className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {startHereVideos.map((entry, idx) => (
                    <VideoCard key={`start-${entry.link}-${idx}`} entry={entry} source="ChatGPT" />
                  ))}
                </div>
              </section>
            )}
                ...
                <section key={category} className="space-y-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {catEntries.map((entry, idx) => (
                      <VideoCard key={`${entry.link}-${idx}`} entry={entry} source="ChatGPT" />
                    ))}
                  </div>
                </section>
              );
