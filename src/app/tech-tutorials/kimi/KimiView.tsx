...
const CATEGORIES = [
  ALL,
  "Beginner / Start Here",
  "Core Chat, Writing & Prompting",
  "Long Context, PDF & Research",
  "Coding, Dev & API",
...
export default function KimiView({ entries }: Props) {
      ...
      <div className="flex-1 min-w-0 px-5 pr-12 py-8 md:px-8 md:pr-14">
        ...
        <div className="mb-6 space-y-1.5">
          ...
          <p className="font-sans text-[12px] text-charcoal/50 max-w-2xl leading-relaxed">
            65 community tutorials covering Kimi basics, long-context workflows, coding, multimodal features, and productivity use cases.
          </p>
        </div>
              ...
              <section className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {startHereVideos.map((entry, idx) => (
                    <VideoCard key={`start-${entry.link}-${idx}`} entry={entry} source="Kimi" />
                  ))}
                </div>
              </section>
            )}
                ...
                <section key={category} className="space-y-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {catEntries.map((entry, idx) => (
                      <VideoCard key={`${entry.link}-${idx}`} entry={entry} source="Kimi" />
                    ))}
                  </div>
                </section>
              );
